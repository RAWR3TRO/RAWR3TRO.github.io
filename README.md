...
Colour sources get `--mono`:

```bash
python3 _build/cutout.py ~/stickers/nightshot.png assets/el/st-nightshot.png --mono --contrast 140
```

The site is strictly black and white — the only colour anywhere is the SMPTE
bars on the boot screen — so a colour source has to be desaturated or it becomes
the loudest thing on the page. It converts on **Rec.709 luma, not a channel
average**: green carries most of the perceived brightness, so averaging flattens
a saturated red and a saturated green to the same grey and throws away the
contrast that made the artwork legible. Desaturating then tends to land
everything in a narrow mid band, which is what `--contrast` (percent, pivoting
on mid-grey) is for; 130–160 usually restores the snap.

Desaturation happens **before** matting, so the flood threshold is measured
against the tones that actually ship — a saturated background can otherwise sit
above the threshold in one channel and below it in another.
...
First load was **19.9 MB** and is now **8.0 MB**, of which only ~4.2 MB is on the
critical path — the rest arrives after the boot screen hands over.

| | before | after | how |
|---|---|---|---|
| `psp.glb` | 12,065 KB | 2,661 KB | textures re-encoded, `_build/optimize_glb.py` |
| `theme.m4a` | 3,908 KB | 3,908 KB, deferred | no `src` until boot completes |
| cut-out PNGs | 2,978 KB | 1,248 KB | greyscale + adaptive filtering, `_build/optimize_png.py` |
| below-fold images | eager | deferred | `loading="lazy"` |

**The model was 61% of the payload and 81% of the model was texture.** All five
maps were 4096x4096 on something that renders about 600px wide — roughly 28x
more texture than any pixel can show. Two were PNG for no reason: a packed
metallic-roughness/occlusion map and a normal map, neither of which carries
alpha. Re-encoded at 1024px, they went 9.56 MB -> 0.38 MB with no visible
difference; the one texture that *does* need alpha is detected from the material
bindings and left untouched.

**The track was being pulled during first paint.** `preload="auto"` is
load-bearing for the muted-buffering trick (see [Music](#music)), so rather than
weaken it the `<audio>` now ships with `data-src` and no `src`, and `main.js`
attaches it at the boot hand-off. Same behaviour, 3.9 MB off the critical path —
it now starts fetching around 16s in rather than at 0.

One consequence worth knowing: "no `src` yet" and "no track at all" became
different states, so the old `networkState === 3` (NO_SOURCE) probe would have
reported every cold page as trackless and deleted the sound toggle. Failure is
now detected only from a real `error` once a src exists.

**The PNGs were storing colour they did not have.** Every prop is monochrome but
they were saved RGBA — three identical channels at 4 bytes a pixel. PNG colour
type 4 (grey+alpha) halves that losslessly. The encoder also now picks a filter
per scanline instead of writing filter 0 everywhere, which is where most of the
rest came from. 55% off, nothing thrown away.

### Cache busting

`css` and `js` are referenced with a `?v=` content hash, regenerated by:

```bash
python3 _build/stamp.py
```

Run it after editing either. This is not theoretical bookkeeping: while tuning
the audio deferral Chrome held a `main.js` from before the change and paired it
with the new HTML, producing a sound toggle that claimed to be on with no audio
behind it and nothing in the console to explain why.

### Still on the table

- The four remaining fonts ship as TTF (152 KB). WOFF2 would be roughly 40%
  smaller but needs `fonttools`/`brotli`, neither of which is on this machine.
- three.js arrives as 13 separate module requests from jsdelivr (288 KB).
  `preconnect` takes the handshake off the critical path; bundling would remove
  the waterfall but needs a build step.
